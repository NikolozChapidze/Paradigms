...
(define (partition pivot num-list comparefn)
  (if (null? num-list) '(() ())
	...
	(if (comparefn (car num-list) pivot)
...
;;
(define (comparefn firstpoint secondpoint)
  (< (car firstpoint) (car secondpoint))
)

(define (intersection-points circles)
  (if (null? circles) '()
    (append 
      (apply append (map (lambda (circleee) (intersect circleee (car circles))) (cdr circles)))
      (intersection-points (cdr circles))
    )
  )
)

(define (distance-product centerPoint points)
  (apply *
    (map 
      (lambda (point) 
        (if(= (dist centerPoint point) 0) 1 
        (dist centerPoint point)
        ) 
      ) 
    points)
  )
...
(define (sort-points points)
  (quicksort points comparefn) 
)

(define (clumped-points points)
  (apply append
    (map (lambda (point) (cdr point)) 
      (prefix-of-list 
...
(define (average-point points)
  (let (
        (x (/ (apply + (map (lambda (point) (car point)) points)) (length points)))
        (y (/ (apply + (map (lambda (point) (cadr point)) points)) (length points)))
       )
    (append 
      (list (distance-product (list x y) points)
        (list x y)
      )
    )
  )
...
(define (where-am-i distances starlocations)
  (sort-points
    (map best-estimate (all-guesses distances starlocations))
  )
)
