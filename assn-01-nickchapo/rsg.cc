...
void createSentence(vector<string>& result, map<string, Definition> &grammar, string key){
  ...
  for (Production::iterator curr = prod.begin(); curr != prod.end(); ++curr) {  
    ...
    //check if non-terminal contonue recursion, in terminal just add to the result vector
    if(nextword[0] == '<' && nextword[nextword.size()-1] == '>'){
      createSentence(result, grammar, nextword);
      continue;
    }
...
bool checkIfSpaceIsNeeded(vector<string>& result, size_t& i){
  if(i!=result.size()-1 && result[i].size()>0){
    if(result[i][result[i].size()-1] == '.' && result[i+1][0] == '.'){
      return true;
    }
    if(result[i+1][0] == ')' && (result[i].size() == 1 || result[i] == "pi" || isdigit(result[i][0]))){
      return true;
    }
    if((!ispunct(result[i+1][0]) || result[i+1][0] == '(' || result[i+1][0] == '-' || result[i+1][0] == '+' ||
        result[i+1][0] == '/' || result[i+1][0] == '*' || result[i+1][0] == '^' || result[i+1][0] == '%'
       || result[i+1][0] == '"' || result[i][0] == '"' || result[i+1][0] == '$' || result[i+1][0] == '_' ) && result[i+1][0]!='\''){
      return true;
    }
  }
  if(i<result.size()-2 && result[i].size() != 1 && result[i+1][0] == '\'' && result[i+2].size() != 1){
    return true;
  }
  if(i<result.size()-1 && result[i+1][0]=='\'' && result[i+1].size() > 1){
    return true;
  }
  return false;
}
...
void printResult(vector<string>& result){
  for(size_t i=0; i<result.size(); i++){
    cout<<result[i];
    if(checkIfSpaceIsNeeded(result,i)){
      cout<<" ";
    }
  }
}
...
int main(int argc, char *argv[])
{
  ...
  for (int i = 1; i <= 3; i++){
    ...
    createSentence(result, grammar, "<start>");
    printResult(result);
